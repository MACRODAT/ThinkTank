"""api/routes/agents.py — Full agent management API."""
...
async def list_agents(dept_id: Optional[str] = None, status: Optional[str] = None):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(f"""
            SELECT a.*,
              (SELECT COUNT(*) FROM agents sub WHERE sub.parent_agent_id=a.id AND sub.status='active') as subordinate_count,
...
async def get_agent(aid: str):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Skills/personality/traits files
        async with db.execute(
            "SELECT * FROM agent_md_files WHERE agent_id=? ORDER BY category, filename",
            (aid,)
        ) as cur:
        ...
        # Subordinates
        async with db.execute(
            "SELECT id, name, role, title, status, hierarchy_level FROM agents WHERE parent_agent_id=?",
            (aid,)
        ) as cur:
            subs = _rows(await cur.fetchall())

        # Recent heartbeats
        async with db.execute(
            "SELECT * FROM agent_heartbeat_log WHERE agent_id=? ORDER BY ran_at DESC LIMIT 5",
            (aid,)
        ) as cur:
            beats = _rows(await cur.fetchall())

    agent["md_files"] = files
    agent["subordinates"] = subs
    agent["recent_heartbeats"] = beats
    return agent
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            ...
            (id,dept_id,name,role,title,is_ceo,hierarchy_level,parent_agent_id,
             personality,tone,heartbeat_interval,model_override,profile_image_url,created_by)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (aid, dept_id.upper(), name, role, title, is_ceo, hierarchy_level,
              parent_agent_id, personality, tone, heartbeat_interval,
              model_override, profile_image_url, created_by))
        await db.commit()
    return {"id": aid}
...
async def update_agent(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        fields = {"name": name, "role": role, "title": title, "personality": personality,
                  "tone": tone, "heartbeat_interval": heartbeat_interval,
...
async def fire_agent(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        # Verify authority: firer must be CEO of same dept or Founder
...
# ══════════════════════════════════════════════════════════════════════════
#  AGENT MARKDOWN FILES (skills, personality, traits, prompts, tone)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
#  DEPARTMENT MD FILES  (guidelines, policy, roles, purpose, charts)
# ══════════════════════════════════════════════════════════════════════════
...
async def founder_inbox(status: Optional[str] = None):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(f"""
            ...
            {where}
            ORDER BY CASE f.priority WHEN 'critical' THEN 1 WHEN 'high' THEN 2 ELSE 3 END, f.created_at DESC
        """, params) as cur:
            return _rows(await cur.fetchall())
...
async def reply_founder_mail(mid: str, reply_body: str = Body(...)):
    ts = datetime.utcnow().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def founder_stats():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT COUNT(*) as c FROM founder_mail WHERE status='unread'") as cur:
            unread = (await cur.fetchone())["c"]
        async with db.execute("SELECT COUNT(*) as c FROM founder_mail WHERE requires_decision=1 AND status != 'replied'") as cur:
            pending_decisions = (await cur.fetchone())["c"]
        async with db.execute("SELECT COUNT(*) as c FROM agent_spawn_requests WHERE status='pending'") as cur:
            spawn_requests = (await cur.fetchone())["c"]
        async with db.execute("SELECT COUNT(*) as c FROM draft_endeavors WHERE status='pending'") as cur:
...
async def request_spawn(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        # Auto-approve if CEO of same dept is requesting
        async with db.execute(
        ...
        # If auto-approved → actually create the agent
        new_agent_id = None
        if auto_status == "approved":
            ...
            """, (new_agent_id, dept_id.upper(), proposed_name, proposed_role,
                   ...
                   requesting_agent_id))
            await db.execute("UPDATE agent_spawn_requests SET approved_by=? WHERE id=?",
...
async def approve_spawn(sid: str, approved_by: str = Body("founder")):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
...
async def reject_spawn(sid: str, reason: str = Body("")):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
# ══════════════════════════════════════════════════════════════════════════
#  DRAFT ENDEAVORS (created by agents)
# ══════════════════════════════════════════════════════════════════════════
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        try:
            ...
            for i, p in enumerate(phases):
                ...
                await db.execute("""
                    INSERT INTO endeavor_phases (id,endeavor_id,name,description,order_index,duration_days)
                    VALUES (?,?,?,?,?,?)
